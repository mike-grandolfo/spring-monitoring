...
public class ServerHome {
	...
	public Server findById(int id) {
		...
		try {
			Server instance = (Server) sessionFactory.getCurrentSession().get("Server", id);
			if (instance == null) {
	...
	public List findByExample(Server instance) {
		...
		try {
			List results = sessionFactory.getCurrentSession().createCriteria("Server").add(Example.create(instance))
					.list();
